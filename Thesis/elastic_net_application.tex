\chapter{Elastic Net and Application}\label{Empirical:Elastic_net}
...
For the weak factor group with strength less than 0.5, Elastic net and Lasso provides similar answer: only choose 2 factors out of 12 candidates.
While when focusing factor with strength above 0.9, Lasso will select almost 2 less factors than the Elastic net.
...
But when facing strong factor with strength above 0.9, Elastic net on average only keep about 36\% factors.

We then compare each single stock to observe how similar the factors selecting decision made by Elastic net and Lasso are agreed with each other.
For every single company, if both Elastic net and Lasso select the same factor (generates factor loading not equals to zero), and disregard the same factor (generate factor loadings equal to zero), we call Elastic net and Lasso made exact agreement of factor selecting.
We also extend our standard of agreement to 90\% interval. 
This means that if the Elastic net and Lasso select and disregard same factors about 90\% of the total factor amount, we call the stock has a 90\% agreement.
